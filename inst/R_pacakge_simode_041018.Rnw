...
lv_mc <- simode(
  ...
  obs_sets=N, simode_ctrl=simode.control(parallel=T))
...
plot(lv_mc, type='est', show='both', plot_mean_sd=T,
     pars_true=c(theta,x0), legend=T)
@
...
simode_fits_multi <- simode(
  ...
  obs=obs, obs_sets=N, decouple_equations=T,
...
plot(lv_force1, type='fit', pars_true=theta, mfrow=c(2,1), legend=T)
@
